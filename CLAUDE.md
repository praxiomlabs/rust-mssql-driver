...
Always Encrypted **decryption (the read path)** is fully implemented via the `always-encrypted` feature with production-ready key providers. **Parameter (write) encryption is implemented for the scalar and temporal types** — `int`, `tinyint`, `smallint`, `bigint`, `bit`, `real`, `float`, `nvarchar`, `varbinary`, `uniqueidentifier`, `date`, `money`, `smallmoney`, `decimal` (via `numeric(value, precision, scale)`), `time`/`datetime2`/`datetimeoffset` (via `time(v, scale)`/`datetime2(v, scale)`/`datetimeoffset(v, scale)`), legacy `datetime` (via `datetime(v)`), `smalldatetime` (via the `SmallDateTime` wrapper), and typed `NULL` (via `null::<T>()`): with `Column Encryption Setting=Enabled`, parameterized queries and `execute` describe their parameters via `sp_describe_parameter_encryption`, encrypt those bound to encrypted columns client-side, and send encrypted RPC parameters (deterministic and randomized). The typed wrappers carry their precision/scale via the `ToSql::encrypted_param_type` hook. Only the fixed-width `char`/`nchar`/`binary` types remain unsupported — they error rather than sending plaintext (tracked in #234; see the `mssql-client::encryption` module docs and LIMITATIONS.md). NOTE: scale-7 temporal forms are byte-exact-validated vs `Microsoft.Data.SqlClient`; lower scales are live-round-trip-validated only (`.NET` defaults temporal params to scale 7 and won't emit lower-scale forms). The key providers:
