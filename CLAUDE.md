...
- **`Authentication`**: `SqlPassword`, `ActiveDirectoryServicePrincipal` (`User Id=<client-id>@<tenant-id>`, `Password=<secret>`), `ActiveDirectoryManagedIdentity`/`ActiveDirectoryMSI` — the Azure AD values log in via the LOGIN7 FEDAUTH feature extension (SecurityToken workflow, #155 Phase 1) and require the `azure-identity` feature. FEDAUTH never goes over plaintext (`no_tls` combinations are rejected). Certificate-based service principals are FEDAUTH too but have **no connection-string keyword** (matching ADO.NET) — set them programmatically with `Credentials::certificate(tenant, client, cert_path, password)` under the `cert-auth` feature (cert is read as PKCS#12 `.pfx`, or a combined PEM by auto-detect; it authenticates to Entra, NOT TDS-level mTLS). The ADAL/MSAL workflows (`ActiveDirectoryPassword`, `Interactive`, …) error with a pointer to #155 Phase 2.
