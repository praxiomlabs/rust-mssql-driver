...
8. **Run the full local CI mirror before every push; never trust a subset.** `cargo check` and unit tests do not catch the `-D warnings`-class failures CI rejects. This repo's gate is reproduced by `just ci-all` (fmt + clippy `--all-features --all-targets -D warnings` + nextest all-features + `cargo doc -D warnings` + examples). `ci-all` does NOT compose three gates CI enforces (spell-check, feature-flag validation, and the live suite), so the real pre-push command is:
   ...
   just ci-all && just typos && just check-feature-flags && \
     MSSQL_HOST=localhost MSSQL_PORT=1433 MSSQL_USER=sa MSSQL_PASSWORD='YourStrong@Passw0rd' \
       ...
       -E 'not (binary(azure_sql) or test(azure_identity_auth) or test(cert_auth) or binary(kerberos_live) or binary(streaming_memory))'
   ```
   ...
   - `streaming_memory` (`crates/mssql-client/tests/streaming_memory.rs`) is the **streaming-redesign Stage 0 falsification harness**: a counting-global-allocator test that asserts peak heap stays ~one row, not the whole response. It **fails on purpose** against today's buffered impl (measured 40 MB peak for a ~10 MB response) and defines "done" for true streaming. Excluded from CI until pull-based streaming lands and makes it pass; run it manually against the local container with `MSSQL_HOST=localhost MSSQL_PORT=1433 MSSQL_USER=sa MSSQL_PASSWORD='YourStrong@Passw0rd' cargo nextest run -p mssql-client --test streaming_memory --run-ignored ignored-only`.
