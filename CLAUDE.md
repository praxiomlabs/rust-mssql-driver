...
8. **Run the full local CI mirror before every push; never trust a subset.** `cargo check` and unit tests do not catch the `-D warnings`-class failures CI rejects. This repo's gate is reproduced by `just ci-all` (fmt + clippy `--all-features --all-targets -D warnings` + nextest all-features + `cargo doc -D warnings` + examples). `ci-all` does NOT compose three gates CI enforces (spell-check, feature-flag validation, and the live suite), so the real pre-push command is:
   ...
   - `streaming_memory` (`crates/mssql-client/tests/streaming_memory.rs`) is the **streaming-redesign memory proof**: a counting-global-allocator test that asserts peak heap stays ~one row, not the whole response, when consuming a ~10 MB result set via `Client::query_stream`. Against the buffered `query` it would peak at ~40 MB; via `query_stream` it measures ~20 KB. It is now in the CI live ignored-run (no longer excluded). The incremental engine is `RowSource` (`row_source.rs`, sans-IO) driving `RowStream` (`row_stream.rs`); the buffered `QueryStream` path is unchanged. Remaining streaming stages (cancel/drop semantics, PLP/BLOB sub-streaming, multi-result-set boundaries) are tracked in the v0.18.0 milestone.
