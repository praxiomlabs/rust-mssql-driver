...
- **`Authentication`**: `SqlPassword`, `ActiveDirectoryServicePrincipal` (`User Id=<client-id>@<tenant-id>`, `Password=<secret>`), `ActiveDirectoryManagedIdentity`/`ActiveDirectoryMSI`, `ActiveDirectoryDefault` (a credential chain: managed identity → signed-in `az`/`azd` CLI session → `Credentials::AzureDefault`) — the Azure AD values log in via the LOGIN7 FEDAUTH feature extension (SecurityToken workflow) and require the `azure-identity` feature. All three FEDAUTH token paths (SP, MI, cert) plus the default chain's CLI leg are live-validated against Azure SQL. FEDAUTH never goes over plaintext (`no_tls` combinations are rejected). Certificate-based service principals are FEDAUTH too but have **no connection-string keyword** (matching ADO.NET) — set them programmatically with `Credentials::certificate(tenant, client, cert_path, password)` under the `cert-auth` feature (cert is read as PKCS#12 `.pfx`, or a combined PEM by auto-detect; it authenticates to Entra, NOT TDS-level mTLS). The interactive ADAL/MSAL values (`ActiveDirectoryPassword`/`Interactive`/`DeviceCodeFlow`) error — `azure_identity` doesn't ship those credentials; acquire the token yourself and pass `Credentials::azure_token` (see the closed #310 for the rationale).
...
2. Miss → `sp_prepexec` (`RpcRequest::prepexec`) prepares and executes in ONE
   round-trip → read `@handle` from the `RETURNVALUE` → store; **`sp_execute`
   params are positional/unnamed** (the server binds by position — naming them
   makes the server reject the stream)
...
`Client::statement_cache_stats()`. A cold miss costs one round-trip via
`sp_prepexec` (#205; the original increment used a two-round-trip
`sp_prepare` + `sp_execute`); the cache then wins by reusing the server-side
handle (`sp_execute` sends only the handle + params, not the full SQL) on
repeated execution — hence opt-in, to gather real numbers.
...
8. **Run the full local CI mirror before every push; never trust a subset.** `cargo check` and unit tests do not catch the `-D warnings`-class failures CI rejects. This repo's gate is reproduced by `just ci-all` (fmt + clippy `--all-features --all-targets -D warnings` + nextest all-features + **doctests (`cargo test --doc --workspace --all-features`)** + `cargo doc -D warnings` + examples). Note: nextest does NOT run doctests, so the `doctest-all` step is what catches broken examples — including the README, which the `doc-tests` crate compiles via `include_str!` (a scoped `cargo test --doc -p <crate>` silently skips it). `ci-all` does NOT compose three gates CI enforces (spell-check, feature-flag validation, and the live suite), so the real pre-push command is:
   ...
   - **Do not share a `CARGO_TARGET_DIR` across worktrees while toggling source.** Building a crate with a temporary source edit (e.g. commenting out a line to prove a test goes red) into a target dir that a later run reuses links the stale artifact — a false green or red. When you toggle source to prove a red, `cargo clean -p <crate>` (or rebuild) before trusting the next run, and prefer a separate target dir per worktree.
   ...
   - The tests excluded from the pre-push command (`azure_sql`, `azure_identity_auth`, `cert_auth`, `kerberos_live`) need infrastructure neither CI nor the local container provides. The first three need a **live Azure SQL environment** / client certificate. One exists (credentials live outside the repo, e.g. a local gitignored `.tmp/azure.env`); run that suite when touching authentication or Azure-path code. The `azure_sql` suite includes live FEDAUTH service-principal login tests (`--features azure-identity`) reading `AZURE_SQL_TENANT_ID`/`AZURE_SQL_CLIENT_ID`/`AZURE_SQL_CLIENT_SECRET` with fallback to the `AZURE_`-prefixed names in the standing env file. It also includes a live **certificate** FEDAUTH login test (`test_azure_ad_certificate_login`, `--features cert-auth,azure-identity`) reading `AZURE_SQL_CLIENT_CERTIFICATE_PATH` (+ optional `…_PASSWORD`; needs a certificate uploaded to the app registration), a **default-chain** test (`test_azure_ad_default_credential_login`, uses the signed-in `az` CLI leg off-Azure), and a **managed-identity** test (`test_azure_ad_managed_identity_login`, needs Azure compute with a system-assigned identity granted a DB user). All four FEDAUTH paths (SP, MI, cert, default chain) are live-validated against Azure SQL.
