...
/// Key derivation salts as specified by Microsoft (dotnet/SqlClient
/// `SqlAeadAes256CbcHmac256EncryptionKey.cs`). Each derived key is
/// `HMAC-SHA256(CEK, UTF-16LE(salt))`; the salt must match the reference
/// byte-for-byte or derived keys (and thus all ciphertext) diverge from
/// every other Always Encrypted client.
...
impl DerivedKeys {
    ...
    ///
    /// Uses HMAC-SHA256 with the Microsoft-specified salts to derive three
    /// sub-keys: `key = HMAC-SHA256(CEK, UTF-16LE(salt))`.
    ///
    /// # Arguments
    ///
    ...
    pub fn derive(cek: &[u8]) -> Result<Self, EncryptionError> {
        ...
        let enc_key = Self::derive_key(cek, ENCRYPTION_KEY_SALT)?;
        let mac_key = Self::derive_key(cek, MAC_KEY_SALT)?;
        let iv_key = Self::derive_key(cek, IV_KEY_SALT)?;
    ...
    /// Derive a single key using HMAC-SHA256 over the UTF-16LE salt.
    fn derive_key(cek: &[u8], salt: &str) -> Result<[u8; AES_KEY_SIZE], EncryptionError> {
        let mut mac = HmacSha256::new_from_slice(cek)
            .map_err(|e| EncryptionError::EncryptionFailed(format!("HMAC init failed: {e}")))?;

        for unit in salt.encode_utf16() {
...
mod tests {
    ...
    /// Expected keys for `test_cek()` per the .NET reference derivation.
    /// Generated by `tests/generate_ae_fixtures.py`; a mismatch here means
    /// the derivation no longer interoperates with other AE clients.
    #[test]
    fn test_key_derivation_spec_vectors() {
        let keys = DerivedKeys::derive(&test_cek()).unwrap();

        let spec_enc_key = [
            0x6c, 0x00, 0x21, 0xc6, 0xbd, 0xb8, 0x6c, 0xa2, 0xbc, 0x0f, 0x82, 0x42, 0x9c, 0x9d,
            0x32, 0x33, 0xc7, 0xc9, 0xb8, 0x5c, 0x2b, 0xba, 0x43, 0xcb, 0xb2, 0xc8, 0xae, 0xa6,
            0xfa, 0x83, 0x01, 0x1f,
        ];
        let spec_mac_key = [
            0xa9, 0x35, 0x1d, 0xf2, 0xfd, 0x2a, 0x87, 0x57, 0x99, 0xd7, 0x9b, 0x04, 0xe6, 0x11,
            0x28, 0x71, 0xed, 0x46, 0x27, 0xa8, 0x36, 0xb3, 0x2c, 0xa1, 0x05, 0xf5, 0x18, 0xa3,
            0xe6, 0x3a, 0x16, 0x4f,
        ];
        let spec_iv_key = [
            0x7b, 0x1e, 0xe9, 0xe7, 0x32, 0x24, 0x48, 0xdb, 0x99, 0x9d, 0x5f, 0xc9, 0x29, 0x47,
            0xb3, 0x6d, 0x7c, 0x03, 0x49, 0x21, 0xec, 0xc5, 0xf9, 0x8e, 0x08, 0x8f, 0xc8, 0x7b,
            0x81, 0x74, 0xb1, 0x2e,
        ];

        assert_eq!(keys.enc_key, spec_enc_key);
        assert_eq!(keys.mac_key, spec_mac_key);
        assert_eq!(keys.iv_key, spec_iv_key);
    }
